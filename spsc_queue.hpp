...
// ringbuffer_base提供一些公共的接口函数
// 但是它并没有提供任何虚函数需要子类继承实现，没有虚函数调用开销
template <typename T>
...
protected:
    // 这个数据成员定义，值得学习
    // cache伪共享，通过CACHELINE (64字节)进行不同线程的修改访问隔离
...
public:
    ...
    spsc_queue(void)
    {
        // 无参数的构造函数，用户在实例化类对象时，必须提供模板参数表示capacity大小
    ...
    {
        // 用`element_count`大小来动态构造队列，之后不会grow
        BOOST_ASSERT(runtime_sized);
    }
