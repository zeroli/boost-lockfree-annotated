...
public:
    ...
    {
        // 无初始大小参数的构造函数，隐式的表明stack内部有自己的capacity，或者从模板参数中获取
    ...
    {
        // 有初始大小参数的构造函数，表明stack没有capacity，
        // 但是仍然有初始大小。一旦到达初始大小了，会自动的grow
        // 当grow时，就不是atomic了，必有一个锁保护的过程
        BOOST_ASSERT(!has_capacity);
        initialize();
    }
...
private:
    ...
    // 这里又采用了padding cacheline来避免cache false sharing
